...
import { CommentField } from "../components/index.js";
import { BlogContext } from "../pages/pages.js";
...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    const handleReplyClick = () => {
        if (!access_token) {
            return toast.error("Log in to reply to a comment.");
        }
    ...
    return (
        <div className="!w-full" style={{ paddingLeft: `${leftVal * 10}px` }}>
            <div className="!my-5 !p-6 rounded-md border border-gray-600">
                ...
                <div className="flex gap-5 items-center !mt-5">

                    {
                        commentData.isReplyLoaded ? 
                            <button className="text-gray-800 p-2 px-3 hover:bg-gray-300/30 rounded-md flex items-center gap-2" onClick={hideReplies}>
                                Hide Reply<i className="fa-regular fa-comment-dots"></i>
                            </button>
                        :
                            <button className="text-gray-800 p-2 px-3 hover:bg-gray-300/30 rounded-md flex items-center gap-2" onClick={loadReplies}>
                                {children.length} Reply<i className="fa-regular fa-comment-dots"></i>
                            </button>
                    }
...

                    {
                        ...
                        <button className="p-2 px-3 rounded-md border border-gray-700 ml-auto hover:bg-red-400/30 hover:text-red-700 flex items-center" onClick={deleteComment}>
                            <i className="fa-regular fa-trash-can pointer-events-none"></i>
                        </button> : ""
                    }
